...
*******************************************************************
* This file is the main component of the PHProxy++ proxy application.
...
if ( $CONFIG['stop_hotlinking'] && empty($_SESSION['no_hotlink']) ) {
	...
	if ( ! empty($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'http') === 0 ) {
		...
		foreach ( array_merge( (array) PHPROXY_URL, $CONFIG['hotlink_domains'] ) as $domain ) {
...
if ($foundPlugin = in_array($URL['domain'], $plugins)) {
	include(PHPROXY_ROOT.'/plugins/'.$URL['domain'].'.php');
}
...
if ( $_SESSION['custom_browser']['referrer'] == 'real' ) {
	...
	if ( isset($_SERVER['HTTP_REFERER']) && $flag != 'norefer' && strpos($tmp = deproxyURL($_SERVER['HTTP_REFERER']), PHPROXY_URL) === false ) {
	...
	******************************************************************/
	...
	private function setCookie($cookieString) {
		...
		if ( isset($attr['domain']) ) {
			...
			# which cookies to send since ".phproxy-plus.sourceforge.net" should match "phproxy-plus.sourceforge.net". It's more
...
if ( $fetch->parseType ) {
	...
	******************************************************************/
	...
	require PHPROXY_ROOT . '/includes/parser.php';
		...
		case 'html':
			...
			if ( $flag != 'frame' && $fetch->sniff == false ) {
				...
				if ( $options['showForm'] ) {
					...
					$vars['proxy']	= PHPROXY_URL; # Base URL for proxy directory
