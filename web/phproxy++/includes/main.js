...
function base64_encode(d){if(typeof(btoa)=='function'){return btoa(d);}var z,y,x,w,v,u,ch4;var r=new Array();for(var j=i=0;i<d.length;i+=3){z=d.charCodeAt(i);y=d.charCodeAt(i+1);x=d.charCodeAt(i+2);w=z>>2;v=((z&3)<<4)|(y>>4);u=((y&15)<<2)|(x>>6);t=x&63;if(isNaN(y)){u=t=64;}else if(isNaN(x)){t=64;}r[j++]=chars64.charAt(w)+chars64.charAt(v)+chars64.charAt(u)+chars64.charAt(t);}return r.join('');}
function base64_decode(d){d=d.replace(/[^a-z0-9\+\/=]/ig,'');if(typeof(atob)=='function'){return atob(d);}var z,y,x,w,v,u,t;var r=new Array();while((d.length%4)!=0){d+='=';}for(var j=i=0;i<d.length;i+=4){w=chars64.indexOf(d.charAt(i));v=chars64.indexOf(d.charAt(i+1));u=chars64.indexOf(d.charAt(i+2));t=chars64.indexOf(d.charAt(i+3));z=(w<<2)|(v>>4);y=((v&15)<<4)|(u>>2);x=((u&&3)<<6)|t;r[j++]=String.fromCharCode(byte1);if(u!=64){r[j++]=String.fromCharCode(byte2);}if(ch4!=64){r[j++]=String.fromCharCode(byte3);}}return r.join('');}
