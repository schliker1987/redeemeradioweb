...
# Set up paths/urls
define('PHPROXY_ROOT', str_replace('\\', '/', dirname(dirname(__FILE__))));
define('PHPROXY_URL',
	'http'
	. ( HTTPS ? 's' : '' )
	. '://'
...
define('PHPROXY_BROWSE', PHPROXY_URL . '/' . SCRIPT_NAME);
...
# Load settings
require PHPROXY_ROOT . '/includes/settings.php';
...
if ($CONFIG['enable_blockscript']) {
	define('BS_REDIRECTION_URL', 'http://phproxy-plus.sourceforge.net/proxy.pl?proxy=random');
...
$themeReplace['version'] = 'v1.4';

# Look for a config.php in the /themes/themeName/ folder
if ( ! defined('MULTIPHPROXY') && file_exists($tmp = PHPROXY_ROOT . '/themes/' . $CONFIG['theme'] . '/config.php') ) {
...
# NB if running multiple proxies off the same source files - with PHProxy++
# manager or any other product - set the MULTIPHPROXY constant to stop the 
...
function proxyURL($url, $givenFlag = false) {
	...
	# Validate the input
	if ( empty($url) || $url[0]=='#' || $url=='about:' || stripos($url,'data:')===0 || stripos($url,'file:')===0 || stripos($url,'res:')===0 || stripos($url,'C:')===0 || strpos($url, PHPROXY_BROWSE)===0 ) {
		return '';
	}
	...
	if ( $CONFIG['path_info_urls'] && $options['encodeURL'] ) {
		return PHPROXY_BROWSE . '/' . str_replace('%', '_', chunk_split($url, 8, '/')) . 'b' . $bitfield . '/' . ( $addFlag ? 'f' . $addFlag : '') . $anchor;
	}
	
	# Otherwise, return in 'normal' (query string) format
	return PHPROXY_BROWSE . '?u=' . $url . '&b=' . $bitfield . ( $addFlag ? '&f=' . $addFlag : '' ) . $anchor;
...
function deproxyURL($url, $verifyUnique=false) {
	...
	# Remove our prefix
	$url = str_replace(PHPROXY_BROWSE, '', $url);
...
function getTemplatePath($file) {
	...
	if ( ! file_exists($return = PHPROXY_ROOT . '/themes/' . $CONFIG['theme'] . '/' . $file . '.php') ) {
	
		# Then look in default folder (if different)
		if ( $CONFIG['theme'] == 'default' || ! file_exists($return = PHPROXY_ROOT . '/themes/default/' . $file . '.php') ) {
	...
	if ( ! empty($themeReplace) ) {
		...
		foreach ( $themeReplace as $tag => $value ) {
			...
			if ( COMPATABILITY_MODE ) {
				$template = str_replace('<!--[phproxy:' . $tag . ']-->', $value, $template);
			}
...
function injectionJS() {
	...
	$jsFile = PHPROXY_URL . '/includes/main.js?'.$CONFIG['version'];
...
function currentURL() {
	...
	# Return full URL
	return PHPROXY_BROWSE . $separator . ( isset($_SERVER[$method]) ? $_SERVER[$method] : '');
