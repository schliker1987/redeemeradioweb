...
*******************************************************************
* This is a stand-alone admin control panel for the PHProxy++ software.
******************************************************************/
...
# this admin script out of the PHProxy++ directory. You can use a relative
# or absolute path to the file.
define('ADMIN_PHPROXY_SETTINGS', 'includes/settings.php');
...
define('ADMIN_VERSION', '1.4');

# Start buffering
ob_start();

# Set up equivalents to PHProxy++'s /includes/init.php constants
# that might be available in the settings file
define('PHPROXY_URL',	pathToURL(dirname(ADMIN_PHPROXY_SETTINGS) . '/..'));
define('PHPROXY_ROOT', str_replace('\\', '/', dirname(dirname(realpath(ADMIN_PHPROXY_SETTINGS)))));

# And backwards compatibility (will be removed at some point)
function findURL() { return PHPROXY_URL; }
define('LCNSE_KEY', '');
define('proxyPATH', PHPROXY_ROOT . '/');

# Load current settings
$settingsLoaded = file_exists(ADMIN_PHPROXY_SETTINGS) && (@include ADMIN_PHPROXY_SETTINGS);

# Extract the "action" from the query string
$action = isset($_SERVER['QUERY_STRING']) && preg_match('#^([a-z-]+)#', $_SERVER['QUERY_STRING'], $tmp) ? $tmp[1] : '';

$cache_bust=filemtime(__FILE__)+filemtime(ADMIN_PHPROXY_SETTINGS);
...
class SkinOutput extends Output {
	...
	protected function wrap() {
		...
		# Append "PHProxy++ control panel" to title
		$title = $this->title . ( $this->title ? ' : ' : '' ) . 'PHProxy++ control panel';
...
OUT;
		...
		echo <<<OUT
				...
				<div id="topheader">
					<h1 id="title">
						<a href="{$self}">PHProxy++ Admin Control Panel</a><br>
...
OUT;
		...
		<div id="footer">
			...
			<div id="footer_bg">
				<p><a href="http://phproxy-plus.sourceforge.net/">PHProxy++</a>&reg; &copy; 2007-2012 PHProxy++. All rights reserved.</p>
			</div>
...
if ( $input->gFetch && $user->isAdmin() ) {
	...
	switch ( $input->gFetch ) {
		...
		case 'news':
			...
			# Connect to PHProxy++
			if ($ch=curl_init('http://phproxy-plus.sourceforge.net/feeds/news.php?vn='.urlencode($CONFIG['version']).'&lk='.urlencode($CONFIG['license_key']).'&cb='.$cache_bust)) {
			...
			if ( empty($success) ) {
				echo 'Currently unable to connect to phproxy-plus.sourceforge.net for a news update.';
			}
...
if ( ! $settingsLoaded ) {
	...
	# Show error and exit
	$error->add('The settings file for PHProxy++ could not be found.
					 Please upload this tool into your root PHProxy++ directory.
					 If you wish to run this script from another location,
					 edit the configuration options at the top of the file.
					 <br><br>
					 Attempted to load: <b>' . ADMIN_PHPROXY_SETTINGS . '</b>');
	$output->out();
...
if ( $user->isAdmin() ) {
	...
	$output->addNavigation('PHProxy++&reg; Licenses', 'https://phproxy-plus.sourceforge.net/purchase.php');
	$output->addNavigation('BlockScript&reg;', $self.'?blockscript');
	$output->addNavigation('Support Forum', 'http://phproxy-plus.sourceforge.net/');
	$output->addNavigation('Promote Your Proxy', 'http://phproxy-plus.sourceforge.net/advertise.shtml');
}
	...
	******************************************************************/
	...
	case 'install':
		...
		if ( $input->pSubmit ) {
			...
			if ( ! $error->hasMsg() && is_writable(ADMIN_PHPROXY_SETTINGS) ) {

				# Load up the file
				$file = file_get_contents(ADMIN_PHPROXY_SETTINGS);
...
# the settings. Useful for storing settings that don't/can't be changed from the control panel\r\n";
				...
				# Save updated file
				if ( file_put_contents(ADMIN_PHPROXY_SETTINGS, $file) ) {
		...
		if ( ! ( $writable = is_writable(ADMIN_PHPROXY_SETTINGS) ) ) {

			$error->add('The settings file was found at <b>' . ADMIN_PHPROXY_SETTINGS . '</b> but is not writable. Please set the appropriate permissions to make the settings file writable.');
	...
	******************************************************************/
	...
	case '':
		...
		if ( ! ( $ok = version_compare($phpVersion, '5', '>=') ) ) {
			$error->add('PHProxy++ requires at least PHP 5 or greater.');
		}
		...
		if ( ! ( $ok = function_exists('curl_version') ) ) {
			$error->add('PHProxy++ requires cURL/libcurl.');
		}
		...
		echo <<<OUT
		<p>This script provides an easy to use interface for managing your PHProxy++. Use the navigation above to get started.</p>
		<h2>Latest PHProxy++ news...</h2>
...
OUT;
		...
		if ( $error->hasMsg() ) {
			echo '<p><span class="bold error-color">Environment check failed</span>. You will not be able to run PHProxy++ until you fix the above issue(s).</p>';
		} else {
			echo '<p><span class="bold ok-color">Environment okay</span>. You can run PHProxy++ on this server.</p>';
		}
		...
		function updateLatestVersion(response) {
			document.getElementById('current-version').innerHTML = '<img src="http://phproxy-plus.sourceforge.net/feeds/proxy-version.php?cb={$cache_bust}" border="0" alt="version" />';
		}
OUT;
		...
		<ul class="green">
			...
			<li>PHProxy++ version: <b>{$proxyVersion}</b></li>
...
OUT;
		...
		# Add footer links
		$output->addFooterLinks('PHProxy++ support forum at SF.net', 'http://phproxy-plus.sourceforge.net/');
...
"\r\n\$CONFIG['options'][" . quote($key) . "] = array(
	...
	'force'	 => $force
			...
			if ( file_put_contents(ADMIN_PHPROXY_SETTINGS, $toWrite) ) {
...
OUT;
		...
		foreach ( $logFiles as $file ) {
			...
			# Verify files is a PHProxy++ log. Log files formatted as YYYY-MM-DD.log
