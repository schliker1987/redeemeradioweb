...
$_labels            = array
                    (
                        ...
                        'show_referer'    => array('Show Referer', 'Pass referring URL'), 
						'a2_encode'       => array('A2 URL', 'Use a2 encodng on the address'), 
                        'rotate13'        => array('Rotate13 URL', 'Use ROT13 URL encoding'), 
                        'base64_encode'   => array('Base64 URL', 'Use base64 URL encoding'),
                        'strip_meta'      => array('Strip Meta', 'Strip meta tags'), 
...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
                        ...
                        . ' <label for="____' . $_config['url_var_name'] . '"><a href="' . $_url . '">Address</a>:</label> <input id="____' . $_config['url_var_name'] . '" type="text" style="width:95%;" name="' . $_config['url_var_name'] . '" value="' . $_url . '" />'
        ...
        $_url_form_iscroll = <<<EOF
        
		
        
EOF;
        
        $_url_form .= $_url_form_iscroll;
...
//$_response_body = str_ireplace("<body", "<body id='wrappera2'", $_response_body);
