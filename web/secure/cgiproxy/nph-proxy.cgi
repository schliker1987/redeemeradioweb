...
#     Requires Perl 5.
#
#   Copyright (C) 1996, 1998-2014 by James Marshall, james@jmarshall.com
...
$PROXY_DIR= '/home/superherofm/anonyweb/web/cgiproxy' ;      # if you need to set it manually
...
$FCGI_SOCKET= '/var/run/fcgiwrap.socket' ; # 8002
...
#    unshift(@headers, "Content-Security-Policy: default-src $THIS_SCRIPT_URL/ 'unsafe-inline' 'unsafe-eval' ; img-src $THIS_SCRIPT_URL/ data: ; form-action $THIS_SCRIPT_URL/ ; base-uri $THIS_SCRIPT_URL/\015\012") ;
    ...
    my $csp_header_value= "default-src $csp_source 'unsafe-inline' 'unsafe-eval' ; img-src $csp_source data: ; font-src $csp_source data:" ;
...
sub footer {
    ...
    my $safety=   $lang eq 'en' || $lang eq ''  ? 'safety'    : $MSG{$lang}{safety} ;
    ...
    my $proxified_homepage= &HTMLescape(full_url("$link_scheme://jmarshall.com/tools/cgiproxy/")) ;
    my $download_link= &HTMLescape(full_url("$link_scheme://jmarshall.com/tools/cgiproxy/releases/cgiproxy.latest.tar.gz")) ;
    my $safety_link= &HTMLescape(full_url("$link_scheme://jmarshall.com/tools/cgiproxy/security.html")) ;
...
<i><a href="$proxified_homepage">CGIProxy $PROXY_VERSION</a>
    ...
    - <a href="$safety_link"><font color=red>$safety</font></a></i>
</td>
<td align=$right>
$rightlink
</td>
	...
	if (ref($in->[$i])) {
		...
		if (@{$in->[$i+2]}==2 and $in->[$i+2][0]=~ /^\d+$/) {
			...
			if ($prefix ne '') {
			    ...
			    $end= $i+2 ;
			    $end+= 2
				while $end<@$in and !($in->[$end]=~ /^[;,]$/)
				    and !($in->[$end-1]=~ $RE_JS_LINE_TERMINATOR
					and $in->[$end-2]=~ m#^(?:\)|\]|\+\+|--)$|
		...
		if (@$block) {   # {} block is not empty
		    $end= 4 ;
		    $end+= 2
			while $end<@$block and !($block->[$end]=~ /^[;,]$/)
			    and !($block->[$end-1]=~ $RE_JS_LINE_TERMINATOR
				and $block->[$end-2]=~ m#^(?:\)|\]|\+\+|--)$|
							    ^(?!(?:case|delete|do|else|in|instanceof|new|typeof|void|function|var)$)
							     (?:[\$_\\0-9'"]|\.\d|/..|\pL)#x ) ;
		...
		while (1) {
		    ...
		    if (!$done) {
			$end+= 2  while $end<@$in and $in->[$end] ne ';' and $in->[$end] ne ','
				      and !($in->[$end-1]=~ $RE_JS_LINE_TERMINATOR 
					    and $in->[$end-2]=~ m#^(?:\)|\]|\+\+|--)$|
								  ^(?!(?:case|delete|do|else|in|instanceof|new|typeof|void|function|var)$)
								   (?:[\$_\\0-9'"]|\.\d|/..|\pL)#x ) ;
	    ...
	    } elsif ($token=~ /^(?:LoadMovie|String|URL|action|addImport|appendChild|background|backgroundImage|baseURI|body|cite|close|codeBase|content|cookie|createElement|cssText|currentSrc|cursor|domain|eval|execCommand|execScript|formAction|frames|getAttribute|getElementById|getElementsByTagName|getPropertyValue|host|hostname|href|innerHTML|insertAdjacentHTML|insertBefore|insertRule|listStyleImage|load|localStorage|location|longDesc|lowsrc|navigate|newURL|nodeValue|oldURL|open|opener|origin|outerHTML|outerText|parent|parentNode|pathname|port|postMessage|poster|profile|protocol|pushState|querySelector|querySelectorAll|referrer|removeChild|replaceChild|replaceState|search|send|sessionStorage|setAttribute|setAttributeNode|setInterval|setNamedItem|setProperty|setRequestHeader|setStringValue|setTimeout|showModalDialog|showModelessDialog|src|text|textContent|toString|top|url|useMap|value|withCredentials|write|writeln)$/)
	    {
		...
		if (!$had_dot and !@$term_so_far and $in->[$i+2] eq ':' and ($i<2 or
		    ...
		    and $token!~ /^(?:location|open|setInterval|setTimeout|frames|parent|top|opener|execScript|navigate|showModalDialog|showModelessDialog|parentWindow|String)$/)
		{
		...
		} elsif (($in->[$i+2] eq '++' or $in->[$i+2] eq '--') and !($in->[$i+1]=~ $RE_JS_LINE_TERMINATOR))
		{
		    ...
		    # Ugly hack to handle e.g. "o.send= function() {...}\n stmt2" .
		    my $done ;
		    if ($in->[$end] eq 'function') {
			$end+= 2 while $end<@$in and $in->[$end] ne '(' ;   # get to "("
			$end+= 12 ;                                         # token after closing "}"
			$done= 1
			    unless $in->[$end] eq '(' or $in->[$end] eq '[' ;
		...
		if ($in->[$i+8] eq '{') {
		    ...
		    push(@$ret, '{', ($with_level  ? ''  : 'var _proxy_jslib_with_objs= [] ; with'),
...
sub parse_js {
	...
	if ($token eq '(' or $token eq '[' or $token eq '{') {
	    ...
	    $div_ok= 1 ;     # we're after closing bracket (unnecessary but faster)
	...
	case 'open':
		...
		return function(method, url, asyncflag, username, password) {
			   ...
			   if (arguments.length==2) {
			       ...
			       return XMLHttpRequest.prototype.open.call(o, method, url, asyncflag, username, password) ;
			   }
		       } ;
...
//alert('starting _proxy_jslib_proxify_js_array_1_0; instanceof String=['+(a[1][0] instanceof String)+'], a[1][0]=['+a[1][0]+']') ;
	...
	a[1][0]= new String(_proxy_jslib_proxify_js(a[1][0]+'()').replace(/\(\)$/, '')) ;
